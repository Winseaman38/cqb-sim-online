...
(function bootstrap() {
  ...
  function updateDebugPanel() {
    const elPlayerId = document.getElementById('dbg-player-id');
    const elRoomCode = document.getElementById('dbg-room-code');
    const elConnected = document.getElementById('dbg-connected');
    const elUnitCount = document.getElementById('dbg-unit-count');
    const elServerUrl = document.getElementById('dbg-server-url');
    if (!elPlayerId) return;

    const pid = window.NETWORK ? NETWORK.getPlayerId() : null;
    elPlayerId.textContent = pid ? pid.slice(0, 8) : '-';
    elRoomCode.textContent = window.NETWORK ? (NETWORK.getRoomCode() || '-') : '-';
    elConnected.textContent = window.NETWORK ? (NETWORK.isConnected() ? 'YES' : 'NO') : '-';
    elUnitCount.textContent = window.UNITS ? UNITS.getAllUnits().length : '-';
    elServerUrl.textContent = window.location.host;
  }
})();
